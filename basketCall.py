...
if __name__ == "__main__":
    ...
    portfolio = torch.sum(x_torch, 1, keepdim=True)
    payoff = torch.maximum(portfolio - dim * strike, torch.zeros_like(portfolio))
    average = torch.mean(payoff, 0, keepdim=True)
    mcprice = np.exp(-r * total_time) * average[:, :, num_time_interval].numpy()
    print(mcprice)
