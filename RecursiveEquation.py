...
class RecursiveEquation(object):
    ...
    def f_torch(self, t, x, y, z, vhat, collateral):
        """Generator function in the PDE."""
        raise NotImplementedError

    def g_torch(self, t, x, vhat, collateral):
...
class FVA(RecursiveEquation):
    ...
    def f_torch(self, t, x, y, z, v_clean, collateral):
        fva = ((self.r_fl - self.rate) * torch.maximum(v_clean - y - collateral, torch.zeros_like(v_clean)) - 
               (self.r_fb - self.rate) * torch.maximum(collateral + y - v_clean, torch.zeros_like(v_clean)))
        discount = -(self.rate) * y

        return fva + discount

    def g_torch(self, t, x, v_clean, collateral):
        return 0.
...
class BCVA(RecursiveEquation):
    ...
    def f_torch(self, t, x, y, z, v_clean, collateral):
        cva = (1.0 - self._recoveryC) * torch.maximum(-v_clean + collateral, torch.zeros_like(v_clean)) * self._lambdaC
        dva = (1.0 - self._recoveryB) * torch.maximum(v_clean - collateral, torch.zeros_like(v_clean)) * self._lambdaB
